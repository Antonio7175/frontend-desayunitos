...
const HistorialAdmin = () => {
  ...
  const fetchComandas = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/comandas/anteriores/admin`, {
        headers: {
  ...
  return (
    <div className="container">
      <h2>Historial de Comandas (Admin)</h2>
      {mensaje && <p className="alert alert-danger">{mensaje}</p>}

      {comandas.length === 0 ? (
        <p>No hay historial de comandas.</p>
      ) : (
